...
```js
let customerSum = await FileAttachment("Project_Proposal/project-fall25-Jordan-NE/METLN/src/data/transactions_clean - transactions.csv").csv({typed: true})
let transactSum = await FileAttachment ("Project_Proposal/project-fall25-Jordan-NE/METLN/src/data/customer_summary_clean.csv").csv({typed: true})
```
...
Table should should have certain stats on the events
    - net earnings
    - total tickets sold
    - price per ticket
    - 
